...
def generate_belt_tooth_profile(z, belt_pitch, pld, hole_width, over_thickness, alpha_deg, num_points=[10,10,5,5]):
    ...
    Corrected timing-belt model (replaces the old perforated-belt approximation):
    - the belt neutral axis imposes the pitch radius: perimeter = belt_pitch * z
    - at the pitch radius an involute at pressure angle alpha starts and runs
      up to PLD + over_thickness above pitch (i.e. past the upper PLD line)
    - from the pitch radius down to pitch radius - PLD the flank is the trochoid
      generated by the belt (same construction as a standard gear, so the foot
      at dedendum is displaced by ~PLD*tan(alpha))
    - the pulley tooth thickness at the pitch circle equals the belt hole width
    ...
    z : int
        Number of teeth
    belt_pitch : float
        Belt pitch = neutral axis pitch, tooth-to-tooth period (mm)
    pld : float
        Pitch line differential = belt thickness / 2 (mm)
    hole_width : float
        Width of the rectangular belt cutout along the pitch line (mm).
        This is the pulley tooth thickness at the pitch circle.
    ...
    --------
    dict containing the 6 profile parts:
        'trochoid_1': [(x1, y1), ...] - First trochoid curve (pitch -> dedendum)
        'involute_1': [(x1, y1), ...] - First involute curve (pitch -> addendum)
        'upper_arc': [(x1, y1), ...] - Upper tip arc
        'involute_2': [(x1, y1), ...] - Second involute curve
        'trochoid_2': [(x1, y1), ...] - Second trochoid curve
        'lower_arc': [(x1, y1), ...] - Lower dedendum arc
    """
    ...
    # Angular dimensions: pulley tooth thickness at pitch = hole width,
    # same convention as the spur gear (thickness / pitch radius).
    angular_tooth_width = hole_width / pitch_radius
    angular_pitch = belt_pitch / pitch_radius  # == 2*pi/z

    # Calculate involute parameters (same as external gear)
    addendum_involute_angle = math.acos(base_radius / addendum_radius)
    max_involute_angle = addendum_involute_angle + involute_function(addendum_involute_angle)
    ...
    # Pointed-tooth guard: tooth must not vanish before the tip
    ...
    trochoid_1 = rotate_profile_part(trochoid_1, rotation_angle)
    ...
    trochoid_2 = rotate_profile_part(trochoid_2, rotation_angle)
...
if __name__ == "__main__":
    ...
    belt = False
    belt_pitch = 5.0       # belt pitch = neutral axis pitch, tooth-to-tooth period (mm)
    belt_pld = 0.5         # PLD = belt thickness / 2 (mm)
    belt_hole = 2.5        # width of the rectangular belt cutout along pitch line (mm)
    belt_over = 0.3        # how much the involute grows past the upper PLD line (mm)
    belt_alpha = 20.0      # pressure angle of the belt / rectangular profile (deg)
    belt_num_points = [10, 10, 5, 5]

    # Generate one tooth profile
    if belt:
        tooth_profile = generate_belt_tooth_profile(z, belt_pitch, belt_pld, belt_hole, belt_over, belt_alpha, num_points=belt_num_points)
    elif external:
    ...
    ax.set_aspect('equal')
    if belt:
        margin = params['addendum_radius'] + belt_pitch
    elif external:
    ...
    ax.set_ylabel('Y (mm)')
    if belt:
        ax.set_title(f'Timing Pulley Profile (z={z}, p={belt_pitch}, PLD={belt_pld}, hole={belt_hole}, over={belt_over}, alpha={belt_alpha}deg)')
    else:
    ...
    if belt:
        print(f"  Belt pitch (neutral axis): {belt_pitch} mm")
        print(f"  PLD: {belt_pld} mm")
        print(f"  Hole width: {belt_hole} mm")
        print(f"  Over thickness: {belt_over} mm")
        print(f"  Belt pressure angle: {belt_alpha}°")
    else:
        print(f"  Module: {m} mm")
